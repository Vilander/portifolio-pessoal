// src/data/searchData.js

export const searchData = [
  {
    title: "Sobre Mim",
    url: "/sobre#bio-pessoal",
    content: `
      Olá mundo, sou Vilander, nascido em novembro de 1986, casado, pai de três filhos e dois cachorros. Meu maior prazer é estar com todos eles curtindo cada momento juntos. Atualmente, estou em transição de carreira para a área de Tecnologia, cursando Ciência de Dados na Univesp e Técnico de Informática para Internet no Senac, com foco em desenvolvimento. Tenho experiência anterior em marketing, design gráfico e gestão de equipes, habilidades que hoje somo ao meu aprendizado em programação, análise de dados e desenvolvimento web. Acredito na importância do aprendizado contínuo e busco sempre evoluir, equilibrando vida pessoal e profissional com muita dedicação e entusiasmo.
    `,
  },
  {
    title: "Formação Acadêmica",
    url: "/sobre#formacao-academica",
    content: `
      
      Formação Acadêmica...
      Instituição: UNIVESP
      Bacharel em Ciência de Dados - UNIVESP (cursando)

      Instituição: UNIVESP
      Técnico em Informática com ênfase em Análise de Dados (dupla certificação) - UNIVESP

      Instituição: SENAC
      Técnico em Informática para Internet - SENAC (cursando)

      Instituição: UNISAL
      Bacharel em Comunicação Social - Publicidade e Propaganda - UNISAL

    `,
  },
  {
    title: "Cursos Complementares",
    url: "/sobre#cursos",
    content: `
      
      Cursos Complementares...
      Imersão Dev 7ª edição - alura
      Imersão FRONT-END 2ª edição - alura
      Fundamentos Power Bi - Fundação Bradesco
      [EVENTO] Codecon Summit - 2024
      Curso on-line de PHP com Laravel - Rockseat
      Introdução à programação - EBAC
      [EVENTO] ERP Summit - 2025
      Criação de site com HTML, CSS e JAVASCRIPT - Escola do Trabalhador 4.0
      Learn to Program - Fiap Nano Courses
      Bootcamp Ri Happy - Front End do zero - Dio.
      Python I - Fundamentos - Senac/Americana-SP
      Bootcamp Suzano Python Developer - Dio.

    `,
  },
  {
    title: "Habilidades Técnicas",
    url: "/sobre#skills",
    content: `
      Habilidades Técnicas/Skills

      Hard Skills:
      Desenvolvimento Web
      HTML | CSS | JavaScript | React | WordPress
      Linguagens de Programação e Dados
      Python | SQL | Power BI | Excel
      Ferramentas de Versionamento e DevOps
      Git | Github | Linux
      Design e Comunicação Visual
      Figma | Photoshop | Illustrator | InDesign | Fotografia
      Pacote Office
      Excel | Word | PowerPoint
      Soft Skills:
      Pensamento Crítico | Curiosidade | Agilidade de Aprendizagem | Perfil Analítico
      Comunicação | Trabalho em Equipe | Gestão de Conflitos | Inteligência Emocional | Liderança
      Organização | Gestão do Tempo | Compromisso com Resultados | Foco em Qualidade | Senso de Urgência
      Proatividade | Criatividade | Adaptabilidade | Resiliência
        
      
    `,
  },
  {
    title: "Experiências Profissionais",
    url: "/sobre#CLT",
    content: `
      Experiências Profissionais...
      Empresa: Safra Bag
      Assistente de Marketing
        
      Empresa: School Picture
      Líder Pré-Impressão
        
      Empresa: Despachante Ajax
      Assitente de escritório
    `,
  },
  {
    title: "Portfólio DEV",
    url: "/portfolio#DEV",
    content: `
      Meu portfólio de desenvolvimento inclui projetos com React, Node.js, APIs REST, e aplicações responsivas.
      Cada projeto reflete minha capacidade de resolver problemas e entregar valor ao usuário final.
    `,
  },
  {
    title: "Marketing Digital",
    url: "/portfolio#MKT",
    content: `
      Também atuo com marketing digital, incluindo estratégias de SEO, campanhas em redes sociais e análise de métricas.
      A combinação entre tecnologia e marketing me permite criar soluções completas.
    `,
  },
  {
    title: "Blog",
    url: "/blog",
    content: `
      No blog compartilho eventos, dicas de desenvolvimento, tendências de tecnologia e notícias relevantes para profissionais da área.

       
      DEVCONVERGE -LATAM 2025:
      A Oracle vai receber em sua casa as comunidades Quarkus Club , SouJava, Java Meetup SP |

      Hackathon Senac:
      Hackathon Senac – Mural de Empreendedorismo Social Tive a oportunidade de participar de um Hackathon |

      ERP Summit 2025:
      Estive presente na ERP Summit 2025
    `,
  },
  {
    title: "Contato",
    url: "/contato#formulario",
    content: `
      Entre em contato comigo através do e-mail [vilander.costa@gmail.com] ou pelas redes sociais. Estou disponível para projetos, parcerias e oportunidades profissionais.

     `,
  },
  {
    title: "Redes Sociais",
    url: "/contato#rede",
    content: `
      Conecte-se comigo nas minhas redes sociais:
      LinkedIn: https://www.linkedin.com/in/vilander-costa/
      Instagram: https://www.instagram.com/vilandercosta/
      GitHub: https://github.com/Vilander
      WhatsApp: https://wa.me/5519993223509
    `,
  },
  {
    title: "Currículo",
    url: "/contato#curriculo",
    content: `
      Faça o Download do meu Currículo e vamos conversar |
      Faça o Download do meu CV para saber mais sobre mim |
      Veja aqui meu curriculum vitae
    `,
  },
];
